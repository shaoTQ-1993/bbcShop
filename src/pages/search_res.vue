<template>
	<div class="search-res">
	
		<x-header style="background-color: #eb6100;">商品</x-header>
		<ul class="com-nav">	
					<li v-bind:class="{active:sorter}">
						<a name="sort" v-on:click="Request(0)">综合
						</a>
					</li>
					<li v-bind:class="{active:sorter_sale}">
						<a name="sort" v-on:click="changeOrder('sales')">销量
							<i v-if="sorter_sale" v-bind:class="[sorttype=='desc'?'down':'up']"></i>
							<i v-else></i>
						</a>
					</li>
					<li v-bind:class="{active:sorter_price}">
						<a name="sort" v-on:click="changeOrder('price')">价格
							<i v-if="sorter_price" v-bind:class="[sorttype=='desc'?'down':'up']"></i>
							<i v-else></i>
						</a>
					</li>
					<li v-bind:class="{active:sorter_time}">
						<a name="sort" v-on:click="changeOrder('time')">最新
							<i v-if="sorter_time" v-bind:class="[sorttype=='desc'?'down':'up']"></i>
							<i v-else></i>
						</a>
					</li>
	
		</ul>
		<div v-if="show" class="car_shadow">
									
									<div class="shadow_cont">
										<div class="shadow_title">
											<a v-on:click="show = false;counte = 1" class="close_btn"></a>
											<p>{{chooseProduct.name}}</p>
											<span v-if='goodsInfo.coupPrice > 0'>{{chooseProduct.coupPrice | currency}}</span>
											<span v-else>{{chooseProduct.price | currency}}</span>
										</div>
										<div class="shadow_spec">
											<p v-for="spec in goodsInfo.specList" class="sp">
												<label>{{spec.spec_name}} ： </label>
												<span v-for="sp in spec.valueList" v-bind:value="sp.spec_value_id" v-on:click="changeClass($event,'act')">{{sp.spec_value}}</span>
											</p>
											<p class="g_num">
												<label>数量 ：</label>
												<span>
													<em v-if="counte > 0" v-on:click="counte -= 1">-</em>
													<em v-else class="act">-</em>
													<i>{{counte}}</i>
													<em v-if="counte < chooseProduct.store" v-on:click="counte += 1">+</em>
													<em v-else class="act">+</em>
												</span>
											</p>
										</div>
										<a v-on:click="addCar(chooseProduct.product_id)" class="addCar">加入购物车</a>
									</div>
								</div>
		<scroller style="margin-top: 46px;" lock-x scrollbar-y :height=showHeight ref="scroller">
			<div class="block">
				<!--商品列表排列方式 sta-->
				
				<!--商品列表排列方式 end-->
				<!--商品列表 sta-->
				
				<article class="active_block" data-scroll="false">
					<div>
						<ul class="go-list" v-if="result">
							<li v-for="goods in webpage.result">
	
								<!--商品详情 sta-->
								<router-link :to="{name:'productDetail',params:{pro_id:goods.goods_id}}" class="del-items">
									<div class="go-item">
										<span class="sq" v-if="goods.store == 0"></span>
										<img v-bind:src="goods.thumbnail" />
										<div class="info_bar">
											<div class="tit">{{goods.name}}</div>
											<div class="price" v-if="goods.coupPrice > 0">{{goods.coupPrice | currency}}</div>
											<div class="price" v-else>{{goods.price | currency}}</div>
										</div>
									</div>
								</router-link>
								<!--商品详情 end-->
	
								<!--购物车弹窗 sta-->
								<a href="#" v-if="goods.store==0" class="car">
									<i></i>
								</a>
								<a href="#" v-else class="car" v-on:click="showCar(goods.goods_id)">
									<i></i>
								</a>
								
								<!--购物车弹窗  end-->
							</li>
						</ul>
						<!--无搜索结果 sta-->
						<div v-else>
							<i class="null-addr"></i>
							<p class="null-txt">暂时没有找到相关单品，请重新搜索</p>
						</div>
						<!--无搜索结果 end-->
					</div>
				</article>
				<!--商品列表 end-->
			</div>
		</scroller>
	</div>
</template>
<script>
import headtitle from "../components/head"
import { Loading, XHeader, Scroller } from 'vux'
export default {
	data() {
		return {
			typeId: '',
			cat: '',
			goodsSelector: '',
			//	sorter: '',
			sorter: true,
			sorttype: 'desc',
			webpage: '',
			result: '',
			num: 0,
			show: false,
			goodsInfo: '',
			counte: 1,
			productList: '',
			chooseProduct: '',

			catId: '',
			pageNo: 2,
			pageSize: 3,

			sorter_sale: false,
			sorter_price: false,
			sorter_time: false,
			index_s: 0,
			keyWord: '',
			quantity: '',
			catName: '',
			showHeight: ''



		}
	},
	components: {
		headtitle,
		Loading,
		XHeader,
		Scroller
	},
	methods: {
		getGoodsList(id) {
			var that = this;
			that.$vux.loading.show({
				text: 'Loading'
			})
			var url = '/rest/goods/getGoodsByCatId';
			var data = {
				catId: id,
				pageNo: 1,
				pageSize: 50
			}

			that.axios.post(url, data)
				.then(response => {
					that.catName = response.data.data.catName;
					that.webpage = response.data.data.page;
					that.result = (response.data.data.page.result.length) > 0;
					that.$nextTick(() => {
						that.$refs.scroller.reset();
					});
					that.$vux.loading.hide();

				})
		},
		Request(index_sort) {
			var that = this;
			that.$vux.loading.show({
				text: 'Loading'
			})
			var url = "/rest/goodsSearch/getList";

			var data = {
				sort: index_sort,
				catId: that.catId,
				keywords: that.keyWord
			}
			if (index_sort == 0)
				that.getGoodsList(that.catId);
				

			else {
				that.axios.post(url, data).then(response => {

					that.webpage = response.data.data.page;
					that.result = (response.data.data.page.result.length) > 0;
					that.$vux.loading.hide();

				})
			}

			that.sorter_sale = false;
			that.sorter_price = false;
			that.sorter_time = false;
			that.sorter = true;
		},
		//初始化页面

		changeOrder(val) {
			var that = this;
			switch (val) {
				case 'sales':

					if (that.sorttype == 'desc') {
						that.sorttype = 'asc';
						that.index_s = 7;
					}
					else {
						that.sorttype = 'desc';
						that.index_s = 6;
					}
					that.Request(that.index_s);
					that.sorter_sale = true;
					that.sorter_price = false;
					that.sorter_time = false;
					that.sorter = false;
					break;
				case 'price':


					if (that.sorttype == 'desc') {
						that.sorttype = 'asc';
						that.index_s = 3;
					}
					else {
						that.sorttype = 'desc';
						that.index_s = 2;
					}
					that.Request(that.index_s);
					that.sorter_sale = false;
					that.sorter_price = true;
					that.sorter_time = false;
					that.sorter = false;
					break;
				case 'time':
					if (that.sorttype == 'desc') {
						that.sorttype = 'asc';
						that.index_s = 9;
					}
					else {
						that.sorttype = 'desc';
						that.index_s = 8;
					}
					that.Request();
					that.sorter_sale = false;
					that.sorter_price = false;
					that.sorter_time = true;
					that.sorter = false;
					break;
			}
		},
		//商品列表排列方式切换

		showCar(goodsId) {
			var that = this;
			that.$vux.loading.show({
				text: 'Loading'
			})
			this.show = true;
			var url = "/rest/goods/" + goodsId;


			that.axios.get(url)
				.then(response => {
					that.goodsInfo = response.data.data;
					that.productList = response.data.data.productList;
					that.chooseProduct = that.productList[that.productList.length - 1];
					console.log(that.productList);
					console.log("ccc",that.chooseProduct);
					that.$nextTick(() => {
						that.$refs.scroller.reset();
					});
					that.$vux.loading.hide();
				})
		},
		//显示弹窗

		changeClass(event, className) {
			var this_ = event.currentTarget;
			$(this_).addClass(className).siblings().removeClass(className);
			//弹窗商品规格样式切换

			var val = $(this_).attr("value");
			for (var i = 0; i < this.productList.length; i++) {
				var specList = this.productList[i].specList;
				for (var j = 0; j < specList.length; j++) {
					if (specList[j].spec_value_id == val) {
						this.chooseProduct = this.productList[i];
						break;
					}
				}
			}
			//找到当前选择的产品

		},
		//弹窗商品规格样式切换

		addCar(product_id) {
			var that = this;
			that.$vux.loading.show({
				text: 'Loading'
			})
			this.show = false;
			var url = "/rest/cart/add";

			var data = {
				productId: product_id,
				num: that.counte,
			}
			that.axios.post(url, data).then(response => {

				this.$vux.alert.show({
					title: '',
					content: '加入购物车成功',
				});
				that.$vux.loading.hide();
			})
		},
		//弹出框里，商品加入购物车

	},
	mounted() {
		this.showHeight = document.documentElement.clientHeight - 46 + 'px';
		var that = this;
		this.active("#category");
		that.catId = this.$route.params.catId;
		that.keyWord = this.$route.params.keyWord;

		this.Request(10);
		this.getGoodsList(that.catId);



	},
	watch: {
		goodsInfo: function () {
			this.$nextTick(function () {
				$('.sp').each(function () {
					$(this).find('span').eq(0).addClass("act");
				});
			})
		}
	}
}
</script>
<style type="text/css">
@important "../../reset.css";

.search-res  ul.com-nav {
	left: 0;
	right: 0;
	display: flex;
	background: #fff;
	border-bottom: 0.1rem solid #eeeeee;
	line-height: 2rem;
	height: 2rem;
	position:fixed;
	z-index: 99;
}

.search-res  .cat_title {

	text-align: center;
	font-size: 15px;
	color: #000000;
	position: absolute;
	top: 38px;
	height: 40px;
	line-height: 40px;
	background-color: #FFFFFF;
	width: 100%;
	margin: 10px 0;
	border-bottom: 1px #eeeeee solid;
}

.search-res  ul.com-nav>li {
	text-align: center;
	display: block;
	-webkit-flex: 1!important;
	-webkit-box-flex: 1;
}

.search-res  ul.com-nav>li>a {
	font-size: .56rem;
	color: #232323;
	display: inline-block;
	position: relative;
	line-height: 1.32rem;
	height: 1.32rem;
}

.search-res  ul.com-nav>li.active>a {
	color: #eb6100;
}

.search-res  ul.com-nav>li>a>i {
	position: relative;
	width: .4rem;
	height: .46rem;
	display: inline-block;
	margin-left: .12rem;
	top: 0.06rem;
}

.search-res  ul.com-nav>li>a>i:before {
	background: url(../imgs/ico_sort.png) no-repeat;
	background-size: 100% auto;
	position: absolute;
	left: 0;
	content: '';
	display: inline-block;
	width: .4rem;
	height: 0.2rem;
	bottom: 0;
	background-position: 0 -0.98rem;
}

.search-res  ul.com-nav>li>a>i.down:before {
	background-position: 0 bottom;
}

.search-res  ul.com-nav>li>a>i:after {
	background: url(../imgs/ico_sort.png) no-repeat;
	background-size: 100% auto;
	position: absolute;
	left: 0;
	content: '';
	display: inline-block;
	width: .4rem;
	height: 0.2rem;
	top: 0;
	background-position: 0 0;
}

.search-res  ul.com-nav>li>a>i.up:after {
	background-position: 0 -0.48rem;
}

.search-res .block article .active_block {
	visibility: visible;
	display: block;
	z-index: 1;
	width: 100%;
	top: 7vh;
	position: absolute;
	box-sizing: border-box;
}

.search-res .block article .active_block p.null-txt {
	font-size: .6rem;
	color: #999;
	text-align: center;
}

.search-res .block article .active_block i.null-addr {
	width: 4.8rem;
	height: 4rem;
	background: url(../imgs/ps_gray_logo.png) no-repeat;
	background-size: 4.8rem auto;
	display: block;
	margin: 4.4rem auto 1.34rem;
}

.search-res .block article.active_block .go-list {
	margin-bottom: 2rem;
	margin-top: 45px;
}

.search-res .block article.active_block .go-list>li {
	height: 7.16rem;
	position: relative;
	background: #fff;
	border-bottom: 1px solid #f1f1f1;
}

.search-res .block article.active_block a.del-items {
	display: block;
	height: 7.16rem;
	position: relative;
	border-bottom: 1px solid #eee;
	padding-top: 0px;
}

.search-res .block article.active_block a.car {
	width: 1.76rem;
	height: 1.76rem;
	background: url(../imgs/cart_pic1.png) no-repeat;
	background-position: 6px 7px;
	background-size: 1.1rem;
	display: inline-block;
	position: absolute;
	top: 3.8rem;
	right: 1.2rem;
	border: 2px solid #FD4433;
	background-color: #FD4433;
	border-radius: 1rem;
}

.search-res .car_shadow {
	
	top: 0;
	left: 0;
	width: 100%;
	height: 100%;
	background-color: rgba(0, 0, 0, .6);
	position: absolute;
	z-index: 99;

}

.search-res .shadow_cont {
	position: absolute;
	background-color: #fff;
	width: 80%;
	margin-top: 190px;
    margin-left: 10%;


}

.search-res .shadow_cont .shadow_title {
	position: relative;
	padding: 0 .96rem;
	margin-bottom: 1.4rem;
	border-bottom: 1px solid #cccccc;
	text-align: left;
}

.search-res .shadow_cont .shadow_title p {
	padding-top: 1.04rem;
	font-size: 0.6rem;
}

.search-res .shadow_cont .shadow_title span {
	height: 1.2rem;
	padding-top: .5rem;
	line-height: .76rem;
	margin-bottom: 0.54rem;
	font-size: .96rem;
	color: #eb6100;
	line-height: .64rem;
	margin-left: -0.12rem;
	display: block;
}

.search-res .shadow_cont .shadow_title .close_btn {
	position: absolute;
	top: .6rem;
	right: .4rem;
	width: .48rem;
	height: .48rem;
	display: block;
	background: url(../imgs/pop-close.png) no-repeat;
	background-size: .48rem auto;
	margin: -0.24rem 0 0 -0.24rem;
}

.search-res .shadow_cont .shadow_spec {
	text-align: left;
	padding: 0 .96rem 1.76rem .96rem;
}

.search-res .shadow_cont .shadow_spec p.sp {
	padding-bottom: 1rem;
	overflow: hidden;
}

.search-res .shadow_cont .shadow_spec p.sp label {
	display: block;
	width: 2.48rem;
	-webkit-box-flex: 1;
	font-size: 0.48rem;
}

.search-res .shadow_cont .shadow_spec p.sp span {
	display: inline-block;
	font-size: .48rem;
	height: 1rem;
	padding: 0 .26rem;
	min-width: 2rem;
	text-align: center;
	line-height: 1rem;
	display: inline-block;
	float: left;
	margin-right: .4rem;
	margin-bottom: 0.2rem;
	border: 1px solid #ccc;
}

.search-res .shadow_cont .shadow_spec p.sp span.act {
	border-color: #eb6100;
	background: #eb6100;
	color: #ffffff;
}

.search-res .shadow_cont .shadow_spec p.g_num label {
	width: 2.48rem;
	-webkit-box-flex: 1;
	font-size: 0.48rem;
	display: block;
}

.search-res .shadow_cont .shadow_spec p.g_num span {
	overflow: hidden;
	display: block;
	margin-bottom: 2rem;
}

.search-res .shadow_cont .shadow_spec p.g_num span i {
	font-style: normal;
	color: #232323;
	box-shadow: none;
	float: left;
	width: 1rem;
	height: 1rem;
	line-height: 1rem;
	font-size: .6rem;
	border: 1px solid #999;
	background: #FFF;
	text-align: center;
	margin: 0 .08rem;
	border-radius: 0;
}

.search-res .shadow_cont .shadow_spec p.g_num span em {
	float: left;
	display: inline-block;
	width: 1rem;
	height: 1rem;
	background: #999;
	text-align: center;
	color: #fff;
}

.search-res .shadow_cont .shadow_spec p.g_num span em.act {
	background-color: #ccc;
}

.search-res .shadow_cont a.addCar {
	width: 100%;
	padding: 0 .3rem;
	line-height: 1.76rem;
	font-size: 0.64rem;
	color: #fff;
	background: #eb6100;
	display: inline-block;
	border: none;
	cursor: pointer;
	text-align: center;
	position: absolute;
	bottom: 0;
	left: 0;
}

.search-res .block article.active_block .go-item {
	background: #fff;
	padding: 0 23px;
	position: relative;
	height: 4.72rem;
}

.search-res .block article.active_block .go-item>img {
	width: 5.8rem;
	height: 5.8rem;
	position: relative;
	top: 0.5rem;
	;
	float: left;
}

.search-res .block article.active_block .go-item .info_bar {
	background: #fff;
	color: #000;
	width: 6.64rem;
	float: right;
}

.search-res .block article.active_block .go-item .info_bar .tit {
	font-size: .64rem;
	display: -webkit-box;
	height: 1.76rem;
	line-height: .9rem;
	margin-top: .86rem;
	text-align: left;
}

.search-res .block article.active_block .go-item .info_bar .price {
	margin-top: 1.7rem;
	margin-left: -.16rem;
	text-align: left;
	font-size: .96rem;
}

.search-res span.pic {
	display: inline-block;
	width: 1.2rem;
	height: 1.4rem;
	background-image: url(../assets/search.png);
	background-repeat: no-repeat;
	background-size: cover;
}

.search-res .header {
	height: 2rem;
	line-height: 2rem;
	background: -webkit-gradient(linear, 0 0, 0 bottom, from(#fff), to(#F0F0F0));
	border-bottom: 1px solid #D7D7D7;
}

.search-res .close-pic {
	background-position: 3px;
	float: left;
	margin-left: .5rem;
	margin-top: .4rem;
}

.search-res .input {
	width: 86%;
	float: right;
	position: relative;
}

.search-res input {
	border-radius: 5px;
	border: 1px solid #ccc;
	height: 1.4rem;
	width: 90%;
	padding-left: 5px;
	position: absolute;
	left: 0;
	top: .3rem;
}

.search-res .search-pic {
	display: inline-block;
	width: 1.4rem;
	height: 1.4rem;
	background-color: #F5F5F5;
	position: absolute;
	right: 10%;
	top: .3rem;
	border: 1px solid #ccc;
	border-top-right-radius: 5px;
	border-bottom-right-radius: 5px;
}

.search-res .search-pic>span {
	background-position: -32px;
}
</style>